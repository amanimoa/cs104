...
DIV::DIV(std::string variableName, int value)
...
DIV::DIV(std::string variableName1, std::string variableName2)
...
void DIV::execute(ProgramState * state, ostream &outf)
{
...
//we need to increment the program counter 
	...
	if(flag_con1){
		...
		{	
			if(it->first == m_variableName1){
				...
				state->set_map(it->first, floor(it->second/m_value));
				break; 		
			}
		}		
	...
	if ((it1 != map_copy.end()) && (it2 != map_copy.end())) //means we found it 
    	...
    	state->set_map(it1->first, floor(it1->second/it2->second));
